...
def main():
    ...
    st.title("⚡ Fortnite Account Sorter Pro")

    uploaded_file = st.file_uploader("📂 Upload Results ZIP file", type="zip")
        ...
        if st.button("🚀 Process Accounts"):
            with st.spinner("Extracting and Processing..."):
                ...
                try:
                    ...
                    with col1:
                        st.download_button("💾 Download Results (CSV)", data=csv_data, file_name=f"results_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv", use_container_width=True)
                        ...
                        if tg_token and tg_chat_id:
                            if st.button("✈️ Send to Telegram", use_container_width=True):
                                with st.spinner("Sending..."):
                                    s = parser.stats
                                    summary_msg = f"📊 *Fortnite Results*\n\n👤 Accounts: `{s['total_accounts']}`\n🔥 Hits: `{s['hit_accounts']}`\n💰 V-Bucks: `{s['total_vbucks']:,}`"
