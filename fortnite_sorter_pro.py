...
class FortniteAccountParser:
    ...
    def parse_line(self, line):
        line = line.strip()
        if not line or line.startswith('#') or 'generated by' in line.lower() or '====' in line: return None
        
        # 1. Credentials (Email:Pass)
        # Using exact logic from old script to identify start of line
        ...
        # 2. Prepare remaining string using Slicing (More accurate than replace)
        # This matches the old script's behavior of looking at everything AFTER the pass
        ...
        # 3. Attributes - Using Old Script Regex Patterns
        ...
        # Vbucks: Old script used group(2) from complex regex, simplified here but keeps logic
        acc['vbucks'] = get_int(r'(?:Vbucks Count|V-Bucks|Vbucks|V_Bucks)[:\s]*([\d,]+)')
        
        # --- SKIN PARSING (EXACT OLD SCRIPT LOGIC) ---
        acc['skins'] = 0
        acc['skin_names'] = []

        # 1. Try to find Count: Skins: [123]
        skins_count_match = re.search(r'Skins:\s*\[(\d+)\]', remaining, re.IGNORECASE)
        if skins_count_match:
            acc['skins'] = int(skins_count_match.group(1))
        
        # 2. Try to find Names: Skins: [123]: Name1, Name2...
        # The key is the colon after the brackets
        ...
        if skins_names_match:
            ...
            # FALLBACK: If the specific [x]: format fails, look for just "Skins: Name1, Name2"
            # This catches logs that might not have the bracketed number
            fallback_match = re.search(r'Skins:(?!\s*\[)(.+?)(?=\s*\||$)', remaining, re.IGNORECASE)
            if fallback_match:
                # Clean up any potential junk if it grabbed a number by mistake
                raw = fallback_match.group(1)
                potential = [n.strip() for n in raw.split(',')]
                # Filter out pure numbers
                acc['skin_names'] = [n for n in potential if not n.isdigit() and len(n) > 2]

        # Sync count if 0 but names exist
