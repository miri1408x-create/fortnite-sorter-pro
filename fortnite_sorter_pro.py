...
# --- Configuration ---
SCRIPT_VERSION = "4.0"  # This forces a cache reset when you update
...
# --- Parser Logic (EXACT COPY OF YOUR OLD SCRIPT LOGIC) ---
class FortniteAccountParser:
    ...
    def parse_line(self, line):
        ...
        # 1. Credentials (Exact Regex from fortnite_sorter.py)
        ...
        def extract(regex, group=1, type='str'):
            m = re.search(regex, remaining, re.IGNORECASE)
            if not m: return 0 if type == 'int' else 'No' if type == 'bool' else 'Unknown'
            val = m.group(group)
            if type == 'bool': return self.normalize_bool(val)
            if type == 'int': return int(val.replace(',', ''))
            return val.strip()

        # 3. Parse Attributes using OLD SCRIPT Patterns
        account['fa'] = extract(r'FA:\s*(Yes|No|True|False|1|0)', 1, 'bool')
        account['twofa'] = extract(r'2FA:\s*(Yes|No|True|False|1|0)', 1, 'bool')
        account['stw'] = extract(r'STW:\s*(Yes|No|True|False|1|0)', 1, 'bool')
        
        # V-Bucks: Handling the specific group logic from your file
        # Old regex: r'(Vbucks Count|V-Bucks|Vbucks):\s*(\d+)' -> Group 2 is number
        vb_match = re.search(r'(Vbucks Count|V-Bucks|Vbucks):\s*([\d,]+)', remaining, re.IGNORECASE)
        account['vbucks'] = int(vb_match.group(2).replace(',', '')) if vb_match else 0
        
        # --- SKINS PARSING (CRITICAL) ---
        # 1. Count: Matches "Skins: [50]"
        skins_count_match = re.search(r'Skins:\s*\[(\d+)\]', remaining)
        account['skins'] = int(skins_count_match.group(1)) if skins_count_match else 0
        
        # 2. Names: Matches "Skins: [50]: Name1, Name2"
        # Using the exact Regex from your old script
        skins_names_match = re.search(r'Skins:\s*\[\d*\]:\s*(.+?)(?=\s*\||$)', remaining)
        
        if skins_names_match:
            # Success! Found names using standard format
            account['skin_names'] = [n.strip() for n in skins_names_match.group(1).split(',')]
        else:
            # Fallback: Sometimes logs miss the brackets or colon.
            # This is a safety net your old script didn't have, but good to keep.
            fallback = re.search(r'Skins:\s*(?![\[])(.+?)(?=\s*\||$)', remaining)
            if fallback:
                potential = fallback.group(1).split(',')
                # Filter out numbers to be safe
                account['skin_names'] = [n.strip() for n in potential if not n.strip().isdigit()]
            else:
                account['skin_names'] = []

        # 3. Other Data
        account['last_played'] = extract(r'Last Played:\s*([^|]+)', 1, 'str')
        account['level'] = extract(r'Level:\s*(\d+)', 1, 'int')
        account['platform'] = extract(r'Platform:\s*([^|]+)', 1, 'str')
        account['matches_played'] = extract(r'Matches(?: Played)?:\s*(\d+)', 1, 'int')
        ...
        for root, _, files in os.walk(root_dir):
            for file in files:
                if file.endswith('.txt'):
                    try:
                        # errors='ignore' IS CRITICAL. 'replace' was breaking the regex matches.
                            ...
                            for line in f:
                                ...
                                if p:
                                    ...
                                    if email not in self.accounts:
                                        ...
                                        # Keep the one with skins/vbucks
                                        if p['vbucks'] > self.accounts[email]['vbucks'] or p['skins'] > self.accounts[email]['skins']:
                                            self.accounts[email] = p
                    except Exception:
                        pass
    ...
    def get_txt_string(self):
        ...
        for acc in sorted_accs:
            ...
            line = (f"{acc['email']}:{acc['password']} | V-Bucks: {acc['vbucks']} | "
                    f"FA: {acc['fa']} | STW: {acc['stw']} | Level: {acc['level']} | "
                    f"{skin_sec} | Last Played: {acc['last_played']}")
            output.write(line + "\n")
...
def main():
    ...
    st.title("⚡ Fortnite Sorter Pro v4.0")
    
    if st.session_state.processed_accounts is None:
        uploaded_file = st.file_uploader("📂 Upload ZIP file", type="zip")
        if uploaded_file and st.button("🚀 Process Accounts"):
