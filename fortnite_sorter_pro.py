...
class FortniteAccountParser:
    ...
    def parse_line(self, line):
        ...
        if not email_pass_match: return None
        
        acc = {
            'email': email_pass_match.group(1).strip(),
            'password': email_pass_match.group(2).strip()
        }
        
        # 2. Prepare remaining string
        # Slicing is safer to preserve data structure
        ...
        acc['twofa'] = get_val(r'(?:2FA|Two Factor)[:\s]*(Yes|No|True|False|1|0)')
        ...
        # A. Get Count: "Skins: [25]" or "Skins: 25"
        count_match = re.search(r'Skins[:\s]*\[?(\d+)\]?', remaining, re.IGNORECASE)
        if count_match:
            acc['skins'] = int(count_match.group(1))

        # B. Get Names: "Skins: [25]: Name1, Name2"
        # We use 'errors=ignore' in file reading so hidden chars won't break this regex.
        # \s* allows for optional spaces between brackets and colon.
        skins_names_match = re.search(r'Skins[:\s]*\[\d*\]\s*:\s*(.+?)(?=\s*\||$)', remaining, re.IGNORECASE)
        ...
        if skins_names_match:
            ...
            # FALLBACK: For logs like "Skins: Name1, Name2" (no [x]: structure)
            # Look for "Skins:" followed by text that DOES NOT start with a number/bracket
            fallback = re.search(r'Skins\s*:\s*(?![\[\d])(.+?)(?=\s*\||$)', remaining, re.IGNORECASE)
            if fallback:
                raw = fallback.group(1)
                names = [n.strip() for n in raw.split(',')]
                # Heuristic: Valid skin names are >2 chars and not digits
                acc['skin_names'] = [n for n in names if len(n) > 2 and not n.isdigit()]

        # Sync count if we found names but count is 0
        ...
        acc['matches_played'] = get_int(r'Matches[:\s]*(\d+)')
        ...
        for root, _, files in os.walk(root_dir):
            for file in files:
                if file.endswith('.txt'):
                    try:
                        # KEY FIX: errors='ignore' drops garbage chars that break regex
